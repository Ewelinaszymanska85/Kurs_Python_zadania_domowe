import os

from aiohttp import web
import asyncpg


async def get_db_pool(app: web.Application) -> None:
    """Tworzy pulę połączeń do bazy przy starcie aplikacji."""
    app["db_pool"] = await asyncpg.create_pool(
        host=os.environ["DB_HOST"],
        port=os.environ.get("DB_PORT", "5432"),
        database=os.environ["DB_NAME"],
        user=os.environ["DB_USER"],
        password=os.environ["DB_PASSWORD"],
    )

    async with app["db_pool"].acquire() as connection:
        await connection.execute(
            """
            CREATE TABLE IF NOT EXISTS books (
                id SERIAL PRIMARY KEY,
                title TEXT NOT NULL,
                author TEXT NOT NULL
            )
            """
        )

    yield

    await app["db_pool"].close()


async def list_books(request: web.Request) -> web.Response:
    """GET /books - zwraca wszystkie książki."""
    pool = request.app["db_pool"]

    async with pool.acquire() as connection:
        rows = await connection.fetch("SELECT id, title, author FROM books ORDER BY id")

    books = [dict(row) for row in rows]

    return web.json_response(books)


async def create_book(request: web.Request) -> web.Response:
    """POST /books - tworzy nową książkę."""
    data = await request.json()

    title = data.get("title")
    author = data.get("author")

    if not title or not author:
        return web.json_response(
            {"error": "Wymagane pola: title, author"}, status=400,
        )

    pool = request.app["db_pool"]

    async with pool.acquire() as connection:
        row = await connection.fetchrow(
            "INSERT INTO books (title, author) VALUES ($1, $2) RETURNING id, title, author",
            title, author,
        )

    return web.json_response(dict(row), status=201)


async def update_book(request: web.Request) -> web.Response:
    """PUT /books/{id} - aktualizuje istniejącą książkę."""
    book_id = int(request.match_info["id"])
    data = await request.json()

    title = data.get("title")
    author = data.get("author")

    if not title or not author:
        return web.json_response(
            {"error": "Wymagane pola: title, author"}, status=400,
        )

    pool = request.app["db_pool"]

    async with pool.acquire() as connection:
        row = await connection.fetchrow(
            "UPDATE books SET title = $1, author = $2 WHERE id = $3 RETURNING id, title, author",
            title, author, book_id,
        )

    if row is None:
        return web.json_response({"error": "Książka nie znaleziona"}, status=404)

    return web.json_response(dict(row))


async def delete_book(request: web.Request) -> web.Response:
    """DELETE /books/{id} - usuwa książkę."""
    book_id = int(request.match_info["id"])
    pool = request.app["db_pool"]

    async with pool.acquire() as connection:
        result = await connection.execute("DELETE FROM books WHERE id = $1", book_id)

    if result == "DELETE 0":
        return web.json_response({"error": "Książka nie znaleziona"}, status=404)

    return web.json_response({"status": "deleted", "id": book_id})


def create_app() -> web.Application:
    app = web.Application()

    app.cleanup_ctx.append(get_db_pool)

    app.router.add_get("/books", list_books)
    app.router.add_post("/books", create_book)
    app.router.add_put("/books/{id}", update_book)
    app.router.add_delete("/books/{id}", delete_book)

    return app


if __name__ == "__main__":
    web.run_app(create_app(), host="0.0.0.0", port=8000)